...
// Updating entire range in an array
// We can do -> Lazy propagation using segment trees
// We want to do queries in log(n) times 

bool canDivide(const vector<int> &nums , int n , int k , int maxSum){
    int currentSum = 0;
    int subarrayCount = 1;
...
int findMinimumLargestSum(const vector<int>&nums , int n , int k){
...
void solve(){
    ...
    // Divide array into k subarray

    cout << findMinimumLargestSum(a , n , k) << nl;
}
