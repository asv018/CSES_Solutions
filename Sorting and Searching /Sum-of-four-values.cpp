...
void solve(){
    ...
    for(int i = 0; i < n ; i++){
        for(int j = i + 1 ; j < n ; j++){
            ...
            // check if (x - currsum) is present in the map
            if(pair_sum.find(x - curr_sum) != pair_sum.end()){
                auto pair = pair_sum[x - curr_sum];
                int k = pair.first;
                int l = pair.second;

                if(k != i and k != j and l != i and l != j){
                    cout << k + 1 << " " << l + 1 << " " << i + 1 << " " << j + 1 << nl;
                    return;
                }
            }
        }
