...
void MaximumSubarraySumII(int n , int a , int b , vector<int> &arr){
    deque<int> dq;
    // to store indices in increasing order of prefix sum values
    ...
    for(int i = 0 ; i <= (n - a) ; i++){
        while(i + b <= n and !dq.empty() and prefixSum[dq.front()] <= prefixSum[i + b]){
            dq.pop_front();
        }
